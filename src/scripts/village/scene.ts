...
// Outdoors a visitor can come within 1.5 m of a detail; zoom follows the pointer (or pinch centre).
const OUTDOOR_MIN_DISTANCE=1.5;
const camera=new T.PerspectiveCamera(38,innerWidth/innerHeight,.2,2600);const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.minDistance=OUTDOOR_MIN_DISTANCE;controls.zoomToCursor=true;controls.maxDistance=1800;controls.maxPolarAngle=Math.PI*.48;controls.minPolarAngle=.04;controls.maxTargetRadius=300;controls.cursor.set(0,0,-60);controls.zoomSpeed=.8;
...
{const agaric:Point=[-75.05,69.65],viewTarget:Point=[-73,70];let p:Point=brooks[0][0],along:Point=[1,0];
 ...
 detailItems=details.items;mount.dataset.hiddenDetails=JSON.stringify(details.where);}
...
function restoreOutdoorControls(){controls.zoomToCursor=true;controls.enableRotate=true;controls.enableDamping=true;controls.panSpeed=1;controls.zoomSpeed=.8;controls.maxDistance=1800;controls.minPolarAngle=.04;controls.maxPolarAngle=Math.PI*.48;camera.near=.2;}
...
function stepInside(h:Home,floor=0){
 ...
 inspection.close(false);tween=null;controls.minDistance=.35;controls.maxDistance=6;controls.zoomToCursor=false;controls.enableRotate=false;controls.enableDamping=false;controls.panSpeed=.16;controls.zoomSpeed=.25;controls.minPolarAngle=.12;controls.maxPolarAngle=Math.PI-.12;camera.near=.04;roomHome=h;$('room-controls').hidden=false;$('village-app').dataset.view='room';
...
const ray=new T.Raycaster();let down:Point=[0,0],clickToken=0;const focusedEvents=new WeakSet<Event>();
// Double-click (or double-tap) to look closely at a spot: the camera glides to about two metres from
// it and orbits round it. Rays test the hidden details and the terrain height field only (most scene
// geometry has released its CPU copy).
function focusAt(clientX:number,clientY:number){
 if(inspection.active||roomHome)return;const rect=renderer.domElement.getBoundingClientRect();
 ray.setFromCamera(new T.Vector2((clientX-rect.left)/rect.width*2-1,-(clientY-rect.top)/rect.height*2+1),camera);
 let point=ray.intersectObjects(detailItems,true)[0]?.point;
 if(!point){const o=ray.ray.origin,d=ray.ray.direction;let prev=0;
  for(let t=.5;t<900;t+=Math.max(.25,t*.02)){if(o.y+d.y*t<ground(o.x+d.x*t,o.z+d.z*t)){let lo=prev,hi=t;for(let i=0;i<24;i++){const m=(lo+hi)/2;if(o.y+d.y*m<ground(o.x+d.x*m,o.z+d.z*m))hi=m;else lo=m;}point=ray.ray.at(hi,new T.Vector3());break;}prev=t;}}
 if(!point)return;
 const back=camera.position.clone().sub(point),eye=point.clone().add(back.setLength(Math.min(back.length(),2.4)));
 eye.y=Math.max(eye.y,point.y+1.1,ground(eye.x,eye.z)+1.25);move(eye,point,false,1.1);
}
{let lastTap=0,lastX=0,lastY=0,start:Point=[0,0];
 renderer.domElement.addEventListener('pointerdown',e=>{start=[e.clientX,e.clientY];});
 renderer.domElement.addEventListener('pointerup',e=>{if(Math.hypot(e.clientX-start[0],e.clientY-start[1])>8)return;const now=performance.now();
  if(now-lastTap<340&&Math.hypot(e.clientX-lastX,e.clientY-lastY)<28){lastTap=0;clickToken++;focusedEvents.add(e);focusAt(e.clientX,e.clientY);}else{lastTap=now;lastX=e.clientX;lastY=e.clientY;}});}renderer.domElement.addEventListener('pointerdown',e=>{down=[e.clientX,e.clientY];});renderer.domElement.addEventListener('pointerup',e=>{if(inspection.active||roomHome||focusedEvents.has(e))return;if(Math.hypot(e.clientX-down[0],e.clientY-down[1])>5)return;
 // A single click waits a moment: the first click of a double-click (look closer) must not select.
 const tap=++clickToken;setTimeout(()=>{if(tap===clickToken)selectAt(e);},300);});
function selectAt(e:PointerEvent){if(inspection.active||roomHome)return;const rect=renderer.domElement.getBoundingClientRect();ray.setFromCamera(new T.Vector2((e.clientX-rect.left)/rect.width*2-1,-(e.clientY-rect.top)/rect.height*2+1),camera);const hit=ray.intersectObjects(pickTargets)[0];if(hit){if(hit.object.userData.forge){selection.visible=false;$('notes-panel').hidden=true;$('notes').setAttribute('aria-expanded','false');$('house-panel').hidden=false;$('house-number').textContent='MUSHROOM GREEN / CHAINSHOP';$('house-name').textContent='The chain workshop';$('enter-house').onclick=()=>{roomHome=null;$('room-controls').hidden=true;rememberPlace({room:null});tween=null;inspection.enter(null,'main');};$('house-description').textContent='A place of fire, iron and hand work. This reconstruction draws on photographs of the surviving Mushroom Green chainshop. Its layout and 1865 appearance are interpreted.';reader.forge();selected=null;$('visit-house').onclick=()=>view('forge');}else{ $('visit-house').onclick=()=>{if(selected){$('house-panel').hidden=true;selection.visible=false;visit(selected);}};select(hit.object.userData.home);}}};controls.addEventListener('start',()=>{tween=null;});renderer.domElement.addEventListener('keydown',e=>{if(e.key==='Home'){e.preventDefault();view('village');}if(e.key==='+'||e.key==='='){e.preventDefault();zoom(.75);}if(e.key==='-'){e.preventDefault();zoom(1/.75);}if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();(e.shiftKey?turn:pan)(e.key==='ArrowLeft'?-1:e.key==='ArrowRight'?1:0,e.key==='ArrowUp'?1:e.key==='ArrowDown'?-1:0);}});document.addEventListener('keydown',e=>{if(e.key==='Escape'){leaveRoom();inspection.close();renderer.domElement.focus();$('house-panel').hidden=true;$('notes-panel').hidden=true;$('notes').setAttribute('aria-expanded','false');selection.visible=false;}});
...
const releaseExclude=new Set<T.Object3D>(pickTargets);detailItems.forEach(o=>o.traverse(c=>releaseExclude.add(c)));
