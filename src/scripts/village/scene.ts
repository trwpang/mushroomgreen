...
const privy=yardPoint(4.8,-6.5);cube(privy.x,privy.y+.9,privy.z,1.2,1.8,1.3,yardMat,founder.angle);cube(privy.x,privy.y+1.85,privy.z,1.45,.12,1.55,clay,founder.angle);
...
treePositions.forEach((p,i)=>{
 ...
 dummy.position.set(p[0],ground(...p),p[1]);dummy.rotation.set(0,woodlandRandom()*Math.PI*2,0);dummy.scale.set(scale*(.94+woodlandRandom()*.22),scale,scale);dummy.updateMatrix();const kind=species[i],treeLeaves=crownMeshes[kind],slot=crownIndex[kind]++;treeLeaves.setMatrixAt(slot,dummy.matrix);treeLeaves.setColorAt(slot,new T.Color().setHSL(.19+woodlandRandom()*.025,.20+woodlandRandom()*.10,.30+woodlandRandom()*.13));
 const base=new T.Vector3(p[0],ground(...p),p[1]);beam(base,base.clone().add(new T.Vector3(.12,5.2*scale,0)),.19*scale,timber);for(let b=0;b<4;b++){const a=b*Math.PI/2+woodlandRandom();beam(base.clone().add(new T.Vector3(0,2.2*scale,0)),base.clone().add(new T.Vector3(Math.cos(a)*1.8*scale,(4+woodlandRandom())*scale,Math.sin(a)*1.8*scale)),.08*scale,timber);}
...
const buildingNames=/^(Masonry|Brickwork|Interior|Shutters|Wood_grain|Shutter_braces|Ironmongery|Window|Door|Threshold|Roof|Ridge|Gutters|Chimney|Flue|Lead|Hearth|Cinders|Hood|Anvil|Finished_chain|Stock_iron|Quench|Tub|Coal_heap|Timber_stock|Bench|Tongs)/;
forge.scene.updateMatrixWorld(true);forge.scene.traverse(o=>{if(o instanceof T.Mesh&&buildingNames.test(o.name)){const mesh=new T.Mesh(o.geometry,o.material);if(/^Cinders/.test(o.name)){mesh.material=(Array.isArray(o.material)?o.material[0]:o.material).clone();const cm=mesh.material as T.MeshStandardMaterial;if(cm.emissiveIntensity>0){cm.emissive.set('#d54e14');cm.emissiveIntensity=.65;}}mesh.applyMatrix4(o.matrixWorld);mesh.castShadow=mesh.receiveShadow=true;forgeRoot.add(mesh);}});scene.add(forgeRoot);const forgePick=new T.Mesh(new T.BoxGeometry(9.7,5.7,5.1),new T.MeshBasicMaterial({visible:false}));forgePick.position.y=2.7;forgePick.userData.forge=true;forgeRoot.add(forgePick);pickTargets.push(forgePick);
...
forge.scene.traverse(o=>{if(o instanceof T.Mesh&&buildingNames.test(o.name)&&! /^(Chimney|Flue|Lead)/.test(o.name)){const mesh=new T.Mesh(o.geometry,o.material);mesh.applyMatrix4(o.matrixWorld);mesh.castShadow=mesh.receiveShadow=true;weaverShop.add(mesh);}});
openStack(weaverShop,-1,4.18,1.25,.66,1.65,.62,distantBrick);
...
function visit(h:Home){const target=new T.Vector3(h.x,h.height+1.5,h.z);const p=localPoint(h,h.number===22?-22:-16,21);const pos=new T.Vector3(p[0],h.height+13,p[1]);if(camera.aspect<.85)pos.sub(target).multiplyScalar(.85/camera.aspect).add(target);move(pos,target);$('place-label').textContent=h.number===22?'Henry Weaver’s home & small chainshop · No. 22':h.household_name+' · No. '+h.number;}
function view(name:string,immediate=false){camera.fov=name==='washing'?62:name==='approach'?34:38;camera.updateProjectionMatrix();$('village-app').dataset.view=name;document.querySelectorAll<HTMLButtonElement>('[data-view]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.view===name)));if(name==='village'){let pos=new T.Vector3(5,655,45);if(camera.aspect<1)pos.sub(center).multiplyScalar(1/camera.aspect*1.16).add(center);move(pos,center,immediate);$('place-label').textContent='Mushroom Green, Black Country';}else if(name==='approach'){const target=new T.Vector3(forgePos[0]+1.2,ground(...forgePos)+1.6,forgePos[1]-1.8);const offset=new T.Vector3(17,2.9,-11).multiplyScalar(Math.max(1,.8/camera.aspect));move(target.clone().add(offset),target,immediate);$('place-label').textContent='The chainshop approach · after the rain';}else if(name==='washing'){const p=localPoint(founder,-1.5,-6.6),q=localPoint(founder,-1.5,-10.5);const target=new T.Vector3(p[0],ground(...p)+1.4,p[1]);move(new T.Vector3(q[0],ground(...p)+3.2,q[1]),target,immediate);$('place-label').textContent='Wash day · Henry Weaver’s yard';}else if(name==='henry')visit(founder);else if(name==='brook'){$('place-label').textContent='Black Brook · water and woodland';const p=brooks[0].reduce((best,p)=>Math.hypot(p[0]+73,p[1]-70)<Math.hypot(best[0]+73,best[1]-70)?p:best,brooks[0][0]);const target=new T.Vector3(p[0],ground(...p),p[1]);const offset=new T.Vector3(9,5.2,13).multiplyScalar(Math.max(1,.8/camera.aspect));move(target.clone().add(offset),target,immediate);}else if(name==='forge'){$('place-label').textContent='Mushroom Green chainshop';const target=new T.Vector3(forgePos[0],ground(...forgePos)+1.5,forgePos[1]);move(target.clone().add(new T.Vector3(16,3.6,-4).multiplyScalar(Math.max(1,.8/camera.aspect))),target,immediate);}else {$('place-label').textContent='Along Mushroom Green lane';move(new T.Vector3(62,ground(8,-8)+33,42),new T.Vector3(8,ground(8,-8)+1,-8));}}
function select(h:Home,go=false){selected=h;$('house-panel').hidden=false;$('notes-panel').hidden=true;$('notes').setAttribute('aria-expanded','false');$('house-number').textContent='HOUSEHOLD / '+String(h.number).padStart(2,'0');$('house-name').textContent=h.household_name.replace(/ 1[78]\d\d/g,'');$('house-description').textContent=`${h.occupants_1861??'Recorded'} ${h.occupants_1861===1?'person':'people'} recorded in 1861. ${h.family} family.${h.number===22?' Henry’s home and working yard form the first detailed village study.':''} ${h.estimated_position?'The map marks this position as estimated.':'Position follows the family map.'} The building is an artistic interpretation.`;const link=$<HTMLAnchorElement>('house-link');link.href='/households/'+h.slug;selection.position.set(h.x,h.height+.25,h.z);selection.visible=true;$<HTMLSelectElement>('house-select').value=String(h.number);if(go){$('village-app').dataset.view='house';document.querySelectorAll('[data-view]').forEach(b=>b.setAttribute('aria-pressed','false'));visit(h);}}
document.querySelectorAll<HTMLButtonElement>('[data-view]').forEach(b=>b.addEventListener('click',()=>{$('house-panel').hidden=true;selection.visible=false;view(b.dataset.view!);}));$('house-select').addEventListener('change',e=>{const h=homes.find(h=>h.number===Number((e.target as HTMLSelectElement).value));if(h){$('house-link').textContent='Read the family record ↗';$('visit-house').onclick=()=>{if(selected)visit(selected);};select(h,true);};});$('close-house').onclick=()=>{$('house-panel').hidden=true;selection.visible=false;};$('visit-house').onclick=()=>{if(selected)visit(selected);};$('map-toggle').onclick=()=>{mapOpen=!mapOpen;overlay.visible=mapOpen;$('map-toggle').setAttribute('aria-pressed',String(mapOpen));};$('light').onclick=()=>{dusk=!dusk;$('light').textContent=dusk?'Dusk':'Daylight';$('light').setAttribute('aria-pressed',String(dusk));$('village-app').dataset.light=dusk?'dusk':'day';};function pauseUI(){$('motion').textContent=paused?'Play':'Pause';$('motion').setAttribute('aria-pressed',String(paused));}pauseUI();$('motion').onclick=()=>{paused=!paused;pauseUI();};$('notes').onclick=()=>{const open=$('notes-panel').hidden;$('notes-panel').hidden=!open;$('house-panel').hidden=true;$('notes').setAttribute('aria-expanded',String(open));};$('close-notes').onclick=()=>{$('notes-panel').hidden=true;$('notes').setAttribute('aria-expanded','false');};
const ray=new T.Raycaster();let down:Point=[0,0];renderer.domElement.addEventListener('pointerdown',e=>{down=[e.clientX,e.clientY];});renderer.domElement.addEventListener('pointerup',e=>{if(Math.hypot(e.clientX-down[0],e.clientY-down[1])>5)return;const rect=renderer.domElement.getBoundingClientRect();ray.setFromCamera(new T.Vector2((e.clientX-rect.left)/rect.width*2-1,-(e.clientY-rect.top)/rect.height*2+1),camera);const hit=ray.intersectObjects(pickTargets)[0];if(hit){if(hit.object.userData.forge){selection.visible=false;$('notes-panel').hidden=true;$('notes').setAttribute('aria-expanded','false');$('house-panel').hidden=false;$('house-number').textContent='MUSHROOM GREEN / CHAINSHOP';$('house-name').textContent='The chain workshop';$('house-description').textContent='Placed at the modern chainshop pin: 52.4754217, −2.0931022. The cottage previously drawn here is removed. This fixes the landmark position; it does not establish the 1865 use of the mapped household plot.';const link=$<HTMLAnchorElement>('house-link');link.href='/forge';link.textContent='Explore the forge study ↗';selected=null;$('visit-house').onclick=()=>view('forge');}else{ $('house-link').textContent='Read the family record ↗';$('visit-house').onclick=()=>{if(selected)visit(selected);};select(hit.object.userData.home);}}});controls.addEventListener('start',()=>{tween=null;});renderer.domElement.addEventListener('keydown',e=>{if(e.key==='Home'){e.preventDefault();view('village');}if(e.key==='+'||e.key==='='){e.preventDefault();zoom(.75);}if(e.key==='-'){e.preventDefault();zoom(1/.75);}if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();camera.position.sub(controls.target).applyAxisAngle(new T.Vector3(0,1,0),e.key==='ArrowLeft'?-.12:.12).add(controls.target);}});document.addEventListener('keydown',e=>{if(e.key==='Escape'){$('house-panel').hidden=true;$('notes-panel').hidden=true;$('notes').setAttribute('aria-expanded','false');selection.visible=false;}});
...
for(let i=0;i<homes.length;i++){const el=labelEls[i];el.hidden=!mapOpen;if(mapOpen){const h=homes[i],p=new T.Vector3(h.x,h.height+7,h.z).project(camera);el.hidden=p.z>1||p.z< -1||Math.abs(p.x)>1||Math.abs(p.y)>1;el.style.left=(p.x*.5+.5)*mount.clientWidth+'px';el.style.top=(-p.y*.5+.5)*mount.clientHeight+'px';}}
ao.enabled=camera.position.distanceTo(controls.target)<130;aoCamera.copy(camera);aoCamera.layers.set(0);landscape.reflect(renderer,camera,controls.target);composer.render();renderer.shadowMap.autoUpdate=false;
if(savePicture){
