...
let paused=reduced||stillFrame,dusk=false,mapOpen=false,selected:Home|null=null,time=stillFrame?4:0,lightAmount=0;let tween:{from:T.Vector3;to:T.Vector3;fromTarget:T.Vector3;target:T.Vector3;t:number;duration:number}|null=null;
const center=new T.Vector3(5,ground(5,-50),-50);
function move(pos:T.Vector3,target:T.Vector3,immediate=false,duration=1.6){if(immediate||reduced){camera.position.copy(pos);controls.target.copy(target);controls.update();tween=null;}else tween={from:camera.position.clone(),to:pos,fromTarget:controls.target.clone(),target:target.clone(),t:0,duration};}
...
function pan(dx:number,dy:number){
 const next=panDestination(camera,tween?.to??camera.position,tween?.target??controls.target,dx,dy);
...
$('room-cutaway').onclick=()=>{if(!roomHome)return;const h=roomHome;leaveRoom();inspection.enter(h);};
