...
function select(h:Home,go=false){inspection.close();rememberPlace({house:h.number,inside:null,floor:null});selected=h;$('step-inside').hidden=h.number===5;$('house-panel').hidden=false;$('notes-panel').hidden=true;$('notes').setAttribute('aria-expanded','false');$('house-number').textContent='HOUSEHOLD / '+String(h.number).padStart(2,'0');reader.show(h);$('house-panel').focus({preventScroll:true});$('enter-house').onclick=()=>{roomHome=null;$('room-controls').hidden=true;rememberPlace({room:null});tween=null;inspection.enter(h);};selection.position.set(h.x,h.height+.25,h.z);selection.visible=true;$<HTMLSelectElement>('house-select').value=String(h.number);if(go){$('village-app').dataset.view='house';document.querySelectorAll('[data-view]').forEach(b=>b.setAttribute('aria-pressed','false'));visit(h);}}
