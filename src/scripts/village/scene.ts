...
const soundUI=()=>{$('sound').textContent=soundscape.enabled?'Sound on':'Sound off';$('sound').setAttribute('aria-pressed',String(soundscape.enabled));};soundUI();
$('sound').onclick=()=>{soundscape.toggle();soundUI();};$('notes').onclick=()=>{const open=$('notes-panel').hidden;$('notes-panel').hidden=!open;$('house-panel').hidden=true;$('notes').setAttribute('aria-expanded',String(open));};$('close-notes').onclick=()=>{$('notes-panel').hidden=true;$('notes').setAttribute('aria-expanded','false');};
