...
const crownRecords:{kind:number;slot:number;matrix:T.Matrix4;center:T.Vector3;radius:number;hidden:boolean}[]=[];
...
treePositions.forEach((p,i)=>{
 ...
 dummy.position.set(p[0],ground(...p),p[1]);dummy.rotation.set(0,woodlandRandom()*Math.PI*2,0);dummy.scale.set(scale*(.94+woodlandRandom()*.22),scale,scale);dummy.updateMatrix();const kind=species[i],treeLeaves=crownMeshes[kind],slot=crownIndex[kind]++;treeLeaves.setMatrixAt(slot,dummy.matrix);crownRecords.push({kind,slot,matrix:dummy.matrix.clone(),center:new T.Vector3(p[0],ground(...p)+4.6*scale,p[1]),radius:3.3*scale,hidden:false});treeLeaves.setColorAt(slot,new T.Color().setHSL(.19+woodlandRandom()*.025,.20+woodlandRandom()*.10,.30+woodlandRandom()*.13));
...
forge.scene.traverse(o=>{if(o instanceof T.Mesh&&buildingNames.test(o.name)&&! /^(Chimney|Flue|Lead)/.test(o.name)){const mesh=new T.Mesh(o.geometry,o.material);mesh.name=o.name;mesh.applyMatrix4(o.matrixWorld);mesh.castShadow=mesh.receiveShadow=true;weaverShop.add(mesh);}});
...
function view(name:string,immediate=false){if(typeof inspection!=='undefined')inspection.close(false);camera.fov=name==='washing'?62:name==='approach'?34:38;camera.updateProjectionMatrix();$('village-app').dataset.view=name;document.querySelectorAll<HTMLButtonElement>('[data-view]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.view===name)));if(name==='village'){let pos=new T.Vector3(5,655,45);if(camera.aspect<1)pos.sub(center).multiplyScalar(1/camera.aspect*1.16).add(center);move(pos,center,immediate);$('place-label').textContent='Mushroom Green, Black Country';}else if(name==='approach'){const target=new T.Vector3(forgePos[0]+1.2,ground(...forgePos)+1.6,forgePos[1]-1.8);const offset=new T.Vector3(17,2.9,-11).multiplyScalar(Math.max(1,.8/camera.aspect));move(target.clone().add(offset),target,immediate);$('place-label').textContent='The chainshop approach · after the rain';}else if(name==='washing'){const p=localPoint(founder,-1.5,-6.6),q=localPoint(founder,-1.5,-10.5);const target=new T.Vector3(p[0],ground(...p)+1.4,p[1]);move(new T.Vector3(q[0],ground(...p)+3.2,q[1]),target,immediate);$('place-label').textContent='Wash day · Henry Weaver’s yard';}else if(name==='henry')visit(founder,immediate);else if(name==='brook'){$('place-label').textContent='Black Brook · water and woodland';const p=brooks[0].reduce((best,p)=>Math.hypot(p[0]+73,p[1]-70)<Math.hypot(best[0]+73,best[1]-70)?p:best,brooks[0][0]);const target=new T.Vector3(p[0],ground(...p),p[1]);const offset=new T.Vector3(9,5.2,13).multiplyScalar(Math.max(1,.8/camera.aspect));move(target.clone().add(offset),target,immediate);}else if(name==='forge'){$('place-label').textContent='Mushroom Green chainshop';const target=new T.Vector3(forgePos[0],ground(...forgePos)+1.5,forgePos[1]);move(target.clone().add(new T.Vector3(16,3.6,-4).multiplyScalar(Math.max(1,.8/camera.aspect))),target,immediate);}else {$('place-label').textContent='Along Mushroom Green lane';move(new T.Vector3(62,ground(8,-8)+33,42),new T.Vector3(8,ground(8,-8)+1,-8),immediate);}}
...
if(hide!==tree.hidden){tree.hidden=hide;crownMeshes[tree.kind].setMatrixAt(tree.slot,hide?zero:tree.matrix);crownMeshes[tree.kind].instanceMatrix.needsUpdate=true;shadowDirty=true;}}
