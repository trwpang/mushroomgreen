...
export function createInspection(scene:T.Scene,renderer:T.WebGLRenderer,camera:T.PerspectiveCamera,controls:OrbitControls,forge:T.Group,smallForge:T.Group){
 ...
 let mainWorkshop=false,watchingWorker=false;
 const watchButton=document.getElementById('watch-worker')!;
 ...
 function restore(){
  ...
  active=false;workshopActive=false;mainWorkshop=false;watchingWorker=false;watchButton.hidden=true;renderer.localClippingEnabled=oldLocalClipping;toolbar.hidden=true;document.getElementById('village-app')!.removeAttribute('data-interior');renderer.shadowMap.needsUpdate=true;
 }
 function close(returnView=true){if(!active)return;rememberPlace({inside:null,floor:null,worker:null});restore();controls.minDistance=1.5;controls.maxDistance=1800;camera.fov=oldFov;camera.updateProjectionMatrix();if(returnView){camera.position.copy(returnCamera);controls.target.copy(returnTarget);controls.update();}document.getElementById('village-canvas')!.querySelector('canvas')?.focus();}
 ...
 function aim(position:T.Vector3,target:T.Vector3){camera.fov=42;camera.updateProjectionMatrix();camera.position.copy(position).sub(target).multiplyScalar(Math.max(1,.95/camera.aspect)).add(target);controls.target.copy(target);controls.minDistance=1.5;controls.maxDistance=55;controls.update();renderer.shadowMap.needsUpdate=true;}
 function watchWorker(){
  if(!active||!mainWorkshop)return;watchingWorker=!watchingWorker;rememberPlace({worker:watchingWorker?1:null});
  if(watchingWorker){aim(forge.localToWorld(new T.Vector3(2.7,2.15,.10)),forge.localToWorld(new T.Vector3(0,1.05,.38)));label.textContent='The chainmaker · main workshop';}
  else{aim(forge.localToWorld(new T.Vector3(7.2,10.8,-8.1)),forge.localToWorld(new T.Vector3(0,.4,0)));label.textContent='The chain workshop';}
  watchButton.textContent=watchingWorker?'Whole workshop':'Watch chainmaker';
 }
 watchButton.onclick=watchWorker;
 function enter(home:Home|null,workshop:'main'|'small'|null=null){
  ...
  if(home&&!workshop&&home.number!==5){const v=interiors.show(home);isolate(v.group);aim(v.camera,v.target);label.textContent=home.household_name+' · No. '+home.number;floorSelect.replaceChildren(...v.plan.floors.map((f,i)=>new Option((i?'Upstairs · ':'Downstairs · ')+f.name,String(i))));floorSelect.hidden=v.floors===1;document.getElementById('interior-evidence')!.textContent=v.floors===2?'The upper floor and room layout are interpretations, not confirmed for this household.':'Single-level model. The original room layout is unverified.';}
   ...
   // Station props are hidden for the cutaway, but never the working chainmaker's own tools.
   let worker=false;for(let x:T.Object3D|null=o;x;x=x.parent)if(x.name==='Working chainmaker'){worker=true;break;}
   if(!worker&&/^(Roof|Ridge|Gutters|Chimney|Flue|Lead|Annex|Finished_chain|Stock_iron|Quench|Tub|Coal_heap|Timber_stock|Bench|Tongs)/.test(o.name)){hiddenParts.push(o);o.visible=false;}
 ...
 return {enter,close,watchWorker,setFloor:(floor:number)=>{floorSelect.value=String(floor);const v=interiors.setFloor(floor);if(v){aim(v.camera,v.target);rememberPlace({floor});}},get active(){return active;},get workshopActive(){return workshopActive;},update:(time:number)=>interiors.update(time)};
}
