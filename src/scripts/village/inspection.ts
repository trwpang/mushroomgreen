...
export function createInspection(scene:T.Scene,renderer:T.WebGLRenderer,camera:T.PerspectiveCamera,controls:OrbitControls,forge:T.Group,smallForge:T.Group){
 ...
 let active=false,house:Home|null=null,returnCamera=new T.Vector3(),returnTarget=new T.Vector3(),oldFov=38;
 ...
 function restore(){
  ...
  active=false;toolbar.hidden=true;document.getElementById('village-app')!.removeAttribute('data-interior');renderer.shadowMap.needsUpdate=true;
 }
 function close(returnView=true){if(!active)return;restore();controls.minDistance=9;controls.maxDistance=1800;camera.fov=oldFov;camera.updateProjectionMatrix();if(returnView){camera.position.copy(returnCamera);controls.target.copy(returnTarget);controls.update();}document.getElementById('village-canvas')!.querySelector('canvas')?.focus();}
 ...
 function enter(home:Home|null,workshop:'main'|'small'|null=null){
  if(active)close();returnCamera.copy(camera.position);returnTarget.copy(controls.target);oldFov=camera.fov;house=home;
  active=true;toolbar.hidden=false;document.getElementById('village-app')!.dataset.interior='true';document.getElementById('house-panel')!.hidden=true;
  if(home&&!workshop&&home.number!==5){const v=interiors.show(home);isolate(v.group);aim(v.camera,v.target);label.textContent=home.household_name+' · No. '+home.number;floorSelect.replaceChildren(...v.plan.floors.map((f,i)=>new Option(f.name,String(i))));floorSelect.hidden=v.floors===1;}
   ...
   if(o instanceof T.Mesh&&/^(Masonry|Brickwork|Interior_lime|Window|Shutter|Door)/.test(o.name)){
  ...
  });renderer.localClippingEnabled=true;const target=root.localToWorld(new T.Vector3(0,.4,0));aim(root.localToWorld(new T.Vector3(8,12,9)),target);label.textContent=workshop==='small'?'Henry’s small chainshop':'The chain workshop';floorSelect.hidden=true;}
  document.getElementById('leave-interior')!.focus();
 }
 floorSelect.onchange=()=>{const v=interiors.setFloor(Number(floorSelect.value));if(v){aim(v.camera,v.target);}};
 ...
 return {enter,close,get active(){return active;},update:(time:number)=>interiors.update(time)};
}
