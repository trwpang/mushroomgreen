...
export function planDressing(home:Home,plan:InteriorPlan,floor:number,sy:number){
 ...
 function fit(id:InteriorObjectId,a:Furnishing,angle=0,heightScale=1){const b=interiorObject(id).bounds.getSize(new T.Vector3()),swap=Math.abs(Math.sin(angle))>.5;return add(id,a.x,0,a.z,a.id,'core',angle,(swap?a.d:a.w)/b.x,(swap?a.w:a.d)/b.z,heightScale);}
 ...
 for(const a of items){
  if(a.kind==='hearth'){
   ...
   const mantel=add('tinderbox',a.x+a.w/2-.065,1.47,a.z+a.d*.32,a.id+'-mantel','surface',Math.PI/2,.7,.7,.7);
   if(n%2===0)add('candlestick',a.x+a.w/2-.06,1.47,a.z-a.d*.32,a.id+'-mantel','surface',0,.8,.8,.8);
  }else if(a.kind==='table'){
   const p=fit(tables[(Math.floor(n/3)+n)%3],a),top=.783;
   if([12,22,40].includes(n)){
    surface('hand-machine',p,top,a.w,a.d,-a.w*.16,0,.9);
    surface('sewing-basket',p,top,a.w,a.d,a.w*.33,0,.80);
   }else{
    ...
    const meal=meals[n%6];surface(meal[0],p,top,a.w,a.d,-a.w*.29,0,.9);surface(meal[1],p,top,a.w,a.d,a.w*.27,-.05,.9);surface(meal[2],p,top,a.w,a.d,0,.14,.9,Math.PI/2);
    // Household work changes the remaining usable tabletop, without stacking unrelated objects.
    const extras:InteriorObjectId[]=['salt-cellar','spice-jar','mortar','coffee-mill','pincushion','scissors','thread-spools','needle-case','darning-mushroom','book','candle-snuffer','soap-dish'];
    surface(extras[n%extras.length],p,top,a.w,a.d,0,-.19,.82);
   }
  }else if(a.kind==='bed'){
   ...
   const id=storage[(n+floor)%3],p=fit(id,a,a.z>0?Math.PI:0);
   if(id==='dresser'){
    ...
    pairs[(n+floor)%5].forEach((item,j)=>surface(item,p,high,a.w,a.d,(j-.5)*a.w*.46,0,.7));
   }
  }else if(a.kind==='stool'){fit((['stool','spindle-chair','ladder-chair'] as InteriorObjectId[])[n%3],a,a.z>0?Math.PI:0);
  }else if(a.kind==='washstand'){
   const p=fit('washstand',a,a.z>0?Math.PI:0);surface('jug',p,.815,a.w,a.d,a.w*.30,0,.7);
   ...
   else{const p=fit('bench',a);surface('folded-linen',p,.463,a.w,a.d,-a.w*.23,0,.8);surface('sewing-basket',p,.463,a.w,a.d,a.w*.23,0,.9);}
  }
 }
 ...
 for(let level=0;level<3;level++){
  ...
  for(let j=0;j<2;j++)surface(level===2?(j?'side-plate':'dinner-plate'):shelfChoices[(n*3+floor*11+level*2+j)%shelfChoices.length],shelf,[.057,.347,.677][level],.72,.23,(j-.5)*.34,0,.64);
 }
 ...
 for(let i=0;i<2;i++){const id=wallItems[(n*2+floor*3+i)%wallItems.length],b=interiorObject(id).bounds;const x=positions[i][0];
  ...
  if(['toast-fork','poker','tongs','coal-shovel','bellows','ladle'].includes(id))add(id,-w/2+.185-b.min.z*.82,.65,-d*.28+i*.16,'hearth-tools','wall',Math.PI/2,.82,.82,.82);
