...
const show=(id:string)=>{group.clear();document.querySelectorAll<HTMLButtonElement>('[data-prop]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.prop===id)));const keys=Object.keys(propNames) as PropKind[];
 if(['all','original','new'].includes(id)){const selected=id==='original'?keys.slice(0,10):id==='new'?keys.slice(10):keys;const rows=Math.ceil(selected.length/5);selected.forEach((k,i)=>{const obj=kit.models[k];obj.position.set((i%5-2)*3.1,0,(Math.floor(i/5)-(rows-1)/2)*3.5);obj.rotation.y=.12;group.add(obj);});camera.position.set(5,rows===4?15:10,rows===4?22:17);controls.target.set(0,.2,0);caption.textContent=(id==='new'?'Ten new objects':id==='original'?'The first ten objects':'Twenty original models')+' · Select an object to inspect its construction';}
 ...
 document.querySelectorAll<HTMLButtonElement>('[data-prop]').forEach(b=>b.onclick=()=>show(b.dataset.prop!));const requested=new URL(location.href).searchParams.get('item');show(requested&&requested in propNames?requested:'new');
