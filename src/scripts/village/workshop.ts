...
export function centralWorkstation(source:T.BufferGeometry,world:T.Matrix4){
 ...
 for(let i=0;i<positions.count;i+=3){const x=(positions.getX(i)+positions.getX(i+1)+positions.getX(i+2))/3;if(Math.abs(x)<1.3)keep.push(i,i+1,i+2);}
