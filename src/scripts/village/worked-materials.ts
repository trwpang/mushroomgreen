...
export function workedMaterials(kind:'clay-oak'|'lime-linen'='clay-oak'){
 const cached=shared.get(kind);if(cached)return cached;
 const ready={value:0};
 const texture=typeof window==='undefined'
  ?new T.DataTexture(new Uint8Array([128,128,128,255]),1,1)
  :new T.TextureLoader().load(`/interior-materials/${kind==='clay-oak'?'worked-clay-oak':'lime-linen'}-v1.webp`,()=>{ready.value=1;});
 texture.colorSpace=T.SRGBColorSpace;
 texture.anisotropy=8;
 texture.wrapS=texture.wrapT=T.ClampToEdgeWrapping;
 const entry={texture,ready};shared.set(kind,entry);return entry;
}
