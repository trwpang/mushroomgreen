...
export function roomCandidates(homes:Home[],camera:T.Vector3,reach=14){
 ...
 }).filter(a=>a.distance<reach&&camera.y<a.home.height+(a.home.style===1?8:6)+(reach-14)*.5&&camera.y>a.home.height-2)
...
export function inhabitHouses(scene:T.Scene,homes:Home[]){
 ...
 const cache=new Map<number,{group:T.Group;rooms:Rooms;used:number}>();let tick=0;let prefetching=false;
 function build(h:Home,visible:boolean){
  const group=new T.Group();group.name=`House ${h.number} furnished floors`;group.visible=visible;root.add(group);
  const rooms=planInterior(h).floors.map((_,floor)=>{const room=createInteriors(group,true);room.show(h,floor);return room;});
  cache.set(h.number,{group,rooms,used:tick});
 }
 function update(camera:T.Vector3,time:number,inspecting:boolean){
  ...
  if(missing){build(missing.home,true);changed=true;}
  // Prefetch the next cottages along the way in idle time, hidden, so panning rarely waits for a build.
  if(!prefetching){const next=roomCandidates(homes,camera,24).find(a=>!cache.has(a.home.number));
   if(next){prefetching=true;const run=()=>{prefetching=false;if(!cache.has(next.home.number))build(next.home,false);};
    if(typeof requestIdleCallback==='function')requestIdleCallback(run,{timeout:700});else setTimeout(run,60);}}
  for(const n of wanted){const e=cache.get(n);if(e){e.used=tick;e.rooms.forEach(r=>r.update(time));}}
  // Keep a small warm cache (visible, prefetched and recently seen) to avoid rebuilding on reversals.
