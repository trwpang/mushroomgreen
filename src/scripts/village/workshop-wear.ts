...
export function wearWorkshop(root:T.Group){
 ...
 root.traverse(o=>{
  ...
  const plaster=/^(Interior_lime|Hood_lime|Hearth_lime)/.test(o.name),floor=/^Interior/.test(o.name)&&!plaster;
  const face=/^Anvil_face/.test(o.name),hood=/^Hood/.test(o.name)&&!plaster;
  const apply=(source:T.Material)=>{
   ...
   m.onBeforeCompile=function(shader,renderer){
    ...
    shader.vertexShader='uniform mat4 shopInverse;varying vec3 shopPoint;varying vec3 shopNormal;\n'+shader.vertexShader;
    shader.vertexShader=shader.vertexShader.replace('#include <project_vertex>','#include <project_vertex>\nshopPoint=(shopInverse*modelMatrix*vec4(transformed,1.)).xyz;shopNormal=mat3(shopInverse*modelMatrix)*normal;');
    shader.fragmentShader=`varying vec3 shopPoint;varying vec3 shopNormal;uniform sampler2D shopAtlas;uniform float shopAtlasReady;
    ...
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
     vec3 swP=shopPoint;
     float swStation=min(abs(swP.x+.43),min(abs(swP.x-2.57),abs(swP.x+3.43)));
     float swHearth=length(vec2(swStation,swP.z-1.6));
     float swAnvil=length(vec2(min(abs(swP.x),min(abs(swP.x-3.),abs(swP.x+3.))),swP.z-.72));
     ...
     ${plaster?`vec3 swN=abs(normalize(shopNormal));vec2 swWall=swN.y>max(swN.x,swN.z)?swP.xz:swN.x>swN.z?swP.zy:swP.xy;
      ...
      diffuseColor.rgb*=1.-swSoot*.35;
      diffuseColor.rgb=mix(diffuseColor.rgb,vec3(.040,.033,.026),swSoot*.14);`:''}
   ...
   m.customProgramCacheKey=()=>key+'|workshop-use-v3-'+[plaster,floor,face,hood].join('-');
   return m;
  };
