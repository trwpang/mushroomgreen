...
for(let i=0;i<=samples;i++){const p=poseAt(i/samples*PERIOD,.125);assert.ok(p.hammerFace.y+.125>=LINK.y+.007-1e-6);for(const a of p.arms){assert.ok(Math.abs(a.shoulder.distanceTo(a.elbow)-.30)<1e-7);assert.ok(Math.abs(a.elbow.distanceTo(a.wrist)-.285)<1e-7);}}
const installed=poseAt(PERIOD*.68,.125);assert.ok(Math.abs(installed.hammerFace.y+.125-(LINK.y+.007))<1e-7,'Raised brick floor must not shift hammer contact');
...
console.log(JSON.stringify({samples,raisedForgeFloor:true,reachableArms:true,constantLimbLengths:true,anvilContact:true,tongContact:true,hammerContact:true,pausedDeterministic:true,continuousLoop:true},null,2));
