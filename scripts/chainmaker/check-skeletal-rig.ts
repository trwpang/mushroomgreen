 ...
 for(let i=0;i<=140;i++){
  ...
  for(const [k,name] of ['RightHand','LeftHand'].entries()){
   ...
   for(const joint of [name,name.replace('Hand','ForeArm'),name.replace('Hand','Arm')]){
    ...
    if(prior){const step=T.MathUtils.radToDeg(q.angleTo(prior));maxJointStep=Math.max(maxJointStep,step);assert.ok(step<9,`No sudden arm rotation: ${joint} moved ${step} degrees in 20 ms`);}
...
console.log(JSON.stringify({frames:282,floors:[0,.125],maxWristError,maxVertex,maxWristBend,maxJointStep,finiteSkin:true},null,2));
