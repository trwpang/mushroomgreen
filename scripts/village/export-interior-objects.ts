...
const doc=new Document(),buffer=doc.createBuffer(),scene=doc.createScene('100 objects of home');
...
assert.equal(report.issues.numErrors,0);assert(bytes.length<6_000_000);assert.equal(decoded.getRoot().listScenes()[0].listChildren().length,100);
...
console.log(JSON.stringify({models:100,triangles,bytes:bytes.length,validationErrors:0}));
