...
// Raised from 180k for the second-pass models (Sept 23); the 6 MB byte budget remains the hard limit.
assert(triangles<240_000);
const sourceFiles=['src/scripts/village/interior-catalogue.ts','src/scripts/village/interior-objects.ts','src/scripts/village/interior-object-detail.ts'];const sourceHashes=Object.fromEntries(await Promise.all(sourceFiles.map(async path=>[path,createHash('sha256').update(await readFile(path)).digest('hex')])));
