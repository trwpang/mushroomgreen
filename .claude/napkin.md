...
- September 22 correction: tool-contact tests alone did NOT certify human movement. Tom correctly identified broken wrists and open grips. Never tie palm orientation directly to a tool mesh up-axis. Use a diagonal palm grip, choose shaft roll by forearm continuity, lower elbow poles, and transport hand roll through the forearm. Refit fingers and rotate the actual tong reins with the grip. Narrow hand weight blending to avoid unrelated fingers pulling the palm. New regression checks bound wrist bend and per-frame joint rotation; actual forge floor must be included. Port 4326 serves built output here, so rebuild before browser review.

- Second September 22 motion review: use Tom's IMG_4265.MOV (3–5 s) for compact strokes, forward lean and the steady holding hand. Small wrist angles alone do not make a natural performance. Added a separate 4.2 s/three-blow skeletal work cycle; legacy demo timing remains unchanged. The old thumb solver reached its target with about −72 degrees of base flexion. Move the thumb contact alongside the curled fingers and bound base extension to −0.35 radians. Limit distal finger curl too. Palm-weight rewrites produced gaps/spikes and were REJECTED; retain the original capsule-weight asset. Browser review must inspect skin, not only bone positions.

- Third September 22 grip correction: arm-side mirroring put the tong palm upwards. Both grip frames now use the overhand palm side, with 95 mm wrist-to-grip reach, 34 mm palm offset and 0.9 rad diagonal rake. Finger fitting must initialize INSIDE its bounds; limiting only candidate steps let an open starting pose survive. New tests check downward palm normals and proximal/middle joint closure over the full stroke. These tests still do not certify close-up skin quality.

- Further left/hammer grip correction: user accepted the tong grip and asked only for the other hand. A shared palm rake put the hammer diagonally through the fingers, with the thumb towards the butt. Use a separate hammer frame/rake and elbow pole, align the actual MCP row with the shaft, and close the thumb across the curled fingers. Terminal finger curl axes must follow the terminal bone direction (parent-to-joint), not the generic palm direction. Tong poses compare exactly unchanged over 421 frames. Do not demand a downward palm for a hammer power grip; validate thumb-towards-head orientation, MCP alignment and thumb-over-finger contact instead.

- September 22 visit pass: IMG_4261–4280 show small pavers, thin limewash on real brick, masonry breasts, square plank workbases and mixed chain sizes. Modern visit evidence is not proof of an 1865 inventory. Preserve the approved worker's anvil face datum. Lime atlas projection must follow each face normal, including breast side faces; fixed XY projection stretches it into stripes. Box coordinates are centres: a .756 high board with .066 thickness has a .789 top. In small-shop cutaways, wall-side props may be fully occluded from the first camera; inspect a rotated overhead view before treating this as a rendering fault. Hide original external yard props with the roof in cutaways so they do not float without ground. New props are instanced and reuse the iron atlas; forge GLB remains below 12 MB.

- IMG_4273 shows a heavy chain-support ring on a slender rod. Retain its timber support in cutaways, keep it to the horn side of the anvil, and preserve the accepted worker motion. Add new seeded props after existing ones to avoid changing their variation.

- September 23 outside road: NLS layer 257 (OS Six Inch 1830s–1880s) places the eastern bend closer to the hamlet than saved modern Quarry Road. Keep original six roads for household orientation; use the traced network for terrain, vegetation and overlay. NLS map-sheet date remains unconfirmed. Retained model brook crosses the new southeast route; bridge form is an explicit interpretation, not established by that map. Test road/stream intersections before calling a road complete. Plane triangles laid along X then Z face down; verify deck normals and open arch with rays. Paint brook banks before roads so they do not erase tracks from a shared bridge-deck atlas.

- September 23 compass: model north is -Z. Derive the screen north bearing from camera-right X/Z, not horizontal view direction, to keep top-down orientation stable. Counter-rotate compass labels and hide with clean view. NLS Map Finder at Tom's point lists Worcestershire IV.NE (101585833), surveyed 1881–82, published 1887: do not treat the 1830s–1880s layer name as proof of an 1865 survey.

- September 23 landscape pass: plan small workshops before terrain and vegetation, then share oriented site/access reservations with props, animals, flowers and plot boundaries. Prepend new shop ground platforms so their own footprint stays level; verify all existing home heights remain unchanged. Individual overlapping roof rows need a small vertical lift to avoid coplanar faces. Most mapped brickkilns lie outside the unchanged scene; do not relocate them. The rail/pit trace uses the later 1881–82 sheet and remains an interpretation. Household labels use HTML buttons, not texture sprites. Current drawCalls telemetry reads only the final composer pass, not whole-frame cost.

- September 23 exterior correction: Tom rejected the alternating pale/red bricks on new workshops. Use exposed weathered brick or timber outside, keep pale limewash at hearth interiors. Historical possibility alone does not support a specific exterior finish; never represent worn limewash by painting whole isolated bricks.

- Codex browser access: bundled browser/26.915.31945/scripts/browser-client.mjs exports setupBrowserRuntime returning an agent. Use the Node REPL tool, then agent.browsers.list/get and browser.tabs.list/get. The signed-in Tripo tab is in iab; Chrome has separate cookies. Do not ask Tom to log into Chrome when his requested Codex tab is available.

- Henry statue: Tripo static model faces +X, not +Z; rotate PI to face kitchen -X. Original model 1.96M triangles reduces to 58K / 0.66 MB. Browser export showed success without a local download; the documented pageAssets inventory exposed the preview GLB after a reload, but bundle rejects kind other. Download that observed asset directly with authorized network access; never regenerate a paid asset merely to retrieve bytes. Metal needs the existing filtered scene reflection to remain readable indoors.

- Three.js skill pack 0.11.0 installed at d1cb23d (24 skills). Review: docs/village/threejs-skills-review.md. Do not assume the whole pack is MIT: some examples are GPL and some sources have no observed licence. Local notes and notices accompany all skills. Fire is WebGPU compute, not drop-in WebGL, and lists timing/emitter/disposal defects. Keep full-village renderer migration separate from a bounded hearth prototype.

- Henry statue placement correction: Tom wants the 18-inch bronze on the floor beside the fireplace, replacing the coal bucket; bucket moves to the other side. Here the original +X front faces the room, so remove the old shelf rotation. Lift dark albedo locally rather than adding emissive glow. Layout checks read dist/households.json: run them AFTER build, never in parallel with a build that replaces dist.

- September 23 landscape pass (Opus): foliage is leaf-cluster cards from one procedural atlas (foliage.ts) on FOLIAGE_LAYER 2 — main/reflection/cube cameras enable it, the GTAO camera must not (its override ignores alpha). Every card geometry needs a `cardNormal` attribute or the edge-on fade makes it invisible. Replaced generators still consume their old RNG draws (7,500 tree, 4,320 shrub); rock thinning happens AFTER the per-rock draws. Canvas-built textures need a DOM-free fallback: node checks import yard-plots. Far country (sky.ts) must take each ring vertex's own ground height near the seam, and smooth edge heights angularly, or lane/brook cuts extrude as radial trenches. The field layout function exists twice (GLSL + JS) and must stay identical. Terrain albedo is tiny (~0.08 linear); match it, not the canvas sRGB. Review with `?cam=`; headless Chrome with --use-angle=metal works when the browser pane is hidden.

- Far country (Sept 23, second pass): built from NLS OS six-inch tiles by scripts/village/build-far-country.py (mapseries-tilesets S3 county layers; zoom 17 ≈ 0.73 m/px; model origin = Henry #22, tile→model is pure scale). Lettering is thick-stroked (EDT ≥ 2.9 px) and must be removed only from the DRAWN hedges — parcels must still be closed by the full ink network, else fields merge. Reservoirs are pale wash, not blue ink. Water ribbons must not use the brook material (planar-reflection feedback loop); they use historic.waterMaterial. bindMaterialAtlas sets uniforms but you must declare `uniform sampler2D x; uniform float xReady;` in GLSL yourself. Limewash must go on AFTER texture-detail's colour layer (inject before roughnessmap_fragment), because each onBeforeCompile replace inserts directly after the include and later callbacks run first.

- Perf (Sept 23): never add/remove lights at runtime — three recompiles EVERY material when light count changes (streamed rooms did this; up to 7 s stalls). Use light-pool.ts requestLight(). Never bake per-house numbers into shader strings/customProgramCacheKey; use uniforms. New material kinds must be covered by the load-time compileAsync warm-up. planInterior is cached and pre-planned in plan-worker.ts. Diagnose with ?debug + renderer.info.programs before/after a pan; getProgramInfoLog in a CPU profile = shader compile stalls.

- Sept 24: fire.ts owns all fires (fuel bed heat field, flame sheets, sparks, hood smoke, pooled light). Forge station centres (forge-local): x -3.39/-0.41/2.57, z≈1.6, bed top y≈.995; Henry's shop keeps only the centre station and its root is scaled (.55,.72,.70) — counter-scale children. Canvas textures used by room code must survive the check scripts' partial DOM shim (guard getImageData). Machine load (VM, iOS simulator) can triple frame times — always A/B against a baseline build under the same load before calling a regression. Tom removed trunk ivy (read as squares). Only other branch was claude/pedantic-shockley-e387fd (April component drafts) — merged into master 24 Sept; its worktree still has someone's uncommitted edits, leave them.

- AAA review loop (Sept 24): `codex exec --skip-git-repo-check -s read-only -i a.jpg -i b.jpg - < prompt.txt` — the prompt MUST come via stdin (`-`), because `-i` swallows following args. Codex reviews take ~5–10 min. Capture with WAIT=10000 or the distant house version can still show. Shared materials: guard any onBeforeCompile injection with a userData flag (glass was injected 3× → GLSL redefinition). Ground shader speckle must test bare soil (r-g), not "not green" — otherwise white daisies everywhere.

- Round 4 (Sept 24): the village camera is clamped ≥1.2 m above ground (animate loop) — judge small props like the cat from that height; for lower debug views disable controls AND raise the clamp. Brook water: one planar mirror serves the whole falling brook — shader mirrors each point to 2·mirrorY−y, and the mirror sits at the LOWEST water within 70 m (else its clip plane cuts bank feet → bright rims). The old ground carve left the ribbon floating 0.45 m over a shallow valley (the "polygonal channel"); the bank profile now climbs through the waterline. ground() changes move stones/reeds heights only (no RNG). Perf rewrites of placement maths must be proven byte-identical (build the old commit in a git worktree, compare outputs) — seeds/layout depend on them. In zsh, `$(ls glob)` inside a command can fail silently; use find.

- Sound (Sept 24): soundscape.ts is fully synthesised Web Audio (brook noise+Minnaert bubbles, bird phrases, modal anvil clang on WORK_STRIKES). AudioContext starts only on first pointer/key (autoplay policy); mute persists in localStorage 'mg-sound'. One-shot PannerNodes must be placed with .value, not setTargetAtTime, or they start at the world origin (silent). Headless tests: tap output by patching AudioNode.prototype.connect in evaluateOnNewDocument; headless frame rate throttles the shared clock (dt capped at .05), so count clangs in a light view (inside=main&house=5).

- Mobile village controls: move existing elements into the More disclosure below 700px, then restore them at comment markers for desktop. Preserve IDs/listeners. Astro can emit small style blocks before the main stylesheet; mobile overrides need explicit root specificity, including hidden toolbar states. The 1440px before/after screenshot matched exactly. Phone overview captures were fog-coloured in the baseline too; forge and interior captures show the geometry.

- Perf (Sept 24, round 5): the village is FILL-bound (GPU time scales with pixels; DPR 1.5→1 saves 15–40%); a foliage depth pre-pass and dropping foliage shadow receive gave nothing. Composer targets have samples=0, so canvas antialias only wasted ~90 MB. InstancedMesh culls as a whole: chunkInstances() splits heavy static sets (crowns/trunks excluded — edited at runtime). Measure GPU with EXT_disjoint_timer_query_webgl2 (Chrome flags --enable-webgl-draft-extensions --enable-privileged-webgl-extensions) around composer.render, interleaving old/new builds; a rAF-spanning probe counts two frames. Lite mode = ?lite or localStorage mg-lite (set automatically on context loss). Dynamic resolution keeps a step only if frames get ≥10% faster. Test draft: `netlify deploy --dir=dist --alias ipad-test` (never --prod without Tom). Known: under heavy load a capture can show shadows from the pre-placement sun (race; not from these changes).

- Round 5 (Sept 25): custom ShaderMaterial with fog:true MUST include THREE.UniformsLib.fog uniforms or render throws every frame (blank canvas). Per-frame multiplicative tweaks of values that are only reset occasionally compound to zero (fog white-out) — keep a base and apply on top. Weather/haze tweaks must keep the subject clear from high overview cameras. Codex's sandbox cannot brew-install; KTX2 needs toktx or a WASM encoder.

- Memory (Sept 25): measure with Chrome memory-infra dumps (Tracing.requestMemoryDump, detailed) — renderer `webgl/buffers` is only GPU accounting (disposing geometry does not lower partition_alloc). Heap snapshot JSArrayBufferData grouped by retainer finds what keeps typed arrays. three.js calls BufferAttribute.prototype.onUploadCallback on every upload; release-cpu.ts patches it to mark uploads, because the env capture uploads geometry before the release pass (onUpload never fires for those). Compute bounds for ALL geometries before dropping any (chunked instances share buffers). Regex /fire/ matched 'Hand-fired brick' — use word boundaries. Audio decodes only after a real pointerdown, so headless element.click() runs have no audio memory. Before calling a shadow difference a regression, force renderer.shadowMap.needsUpdate in both builds: old captures show a stale shadow map (the known race). `rm` in this shell is interactive: use /bin/rm -f.
